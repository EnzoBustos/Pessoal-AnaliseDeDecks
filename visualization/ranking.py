...
from config import ARCHETYPE_RANK_FIGSIZE_MIN_HEIGHT, ARCHETYPE_RANK_FIGSIZE_ROW_HEIGHT, ARCHETYPE_RANK_FIGSIZE_WIDTH, ARCHETYPE_RANK_PLOT_SIZE, FIG_DPI, VIBRANT_CMAP
...
def plot_archetype_rankings(deck_frame: pd.DataFrame, output_dir: Path, top_n: int = ARCHETYPE_RANK_PLOT_SIZE) -> list[Path]:
    ...
    groups = list(deck_frame.groupby("archetype", sort=False))
    for archetype, group in tqdm(groups, desc="Gráficos por arquétipo", unit="arquétipo"):
        ...
        plt.barh(y, scores, color=colors, edgecolor="#0f172a", linewidth=0.8, alpha=0.94)
        plt.errorbar(means, y, xerr=[errors_low, errors_high], fmt="none", ecolor="#fb7185", capsize=4, linewidth=1.1)
        plt.yticks(y, labels)
